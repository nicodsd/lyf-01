...
import { ArrowLeft, MapPin, Award, User, MessageCircle, Sparkles, Send, CreditCard } from 'lucide-react';
...
export const ProviderDetail: React.FC<ProviderDetailProps> = ({ onPaymentSuccess }) => {
  ...
  useEffect(() => {
    if (provider) {
        setGeneratedMessage(`Hola ${provider.name}, vi tu perfil en ServiConnect y estoy interesado en tus servicios.`);
    }
  }, [provider]);
    ...
    <div className="pb-24 bg-white min-h-screen">
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        <div>
          ...
          <p className="text-gray-600 leading-relaxed text-sm">{provider.description}</p>
        </div>
          ...
          <div className="space-y-3">
            {provider.certifications.length > 0 ? (
              provider.certifications.map(cert => (
                <div key={cert.id} className="flex items-start bg-gray-50 p-3 rounded-lg border border-gray-100">
                  <Award className="text-blue-600 mt-1 mr-3 flex-shrink-0" size={20} />
                  <div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 z-40 pb-safe flex gap-3">
        <button
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                ...
                <button 
                  ...
                  className="absolute bottom-2 right-2 bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-md flex items-center gap-1 hover:bg-blue-200 disabled:opacity-50"
                >
                  <Sparkles size={12} />
